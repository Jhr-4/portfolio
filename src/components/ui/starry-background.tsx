...
export function StarryBackground({
  ...
  return (
    <div className="absolute inset-0 pointer-events-none">
      ...
      {/* Small dots/stars - first layer (primary) */}
      {[...Array(starsCount.primary)].map((_, i) => (
        <div 
          key={`star-1-${i}`} 
      ...
      {/* Small dots/stars - second layer (secondary) */}
      {[...Array(starsCount.secondary)].map((_, i) => (
        <div 
          key={`star-2-${i}`} 
      ...
      {/* Small dots/stars - third layer (accent) */}
      {[...Array(starsCount.accent)].map((_, i) => (
        <div 
          key={`star-3-${i}`} 
      ...
      {/* Dust particles - slowly floating */}
      {[...Array(starsCount.dust)].map((_, i) => (
        <div 
          key={`dust-${i}`} 
          className="absolute rounded-full bg-gradient-to-br from-primary/40 to-transparent" 
          style={{
      ...
      <style jsx global>{`
        ...
        .neon-border {
          box-shadow: 0 0 5px #4361EE, 0 0 10px rgba(67, 97, 238, 0.5);
        }
        
        .neon-glow {
          text-shadow: 0 0 2px rgba(67, 97, 238, 0.7), 0 0 5px rgba(67, 97, 238, 0.5);
        }
      `}</style>
    </div>
  );
